...
#The newly-focused parabolic reflector dish is sending all of the collected light to a point on the side of yet another mountain - the l#argest mountain on Lava Island. As you approach the mountain, you find that the light is being collected by the wall of a large facility embedded in the mountainside.
...
# Answer is  Your puzzle answer was 279116

def hash(s):
    initial_value = 0
    for char in s:
        ascii_code = ord(char)
        initial_value += ascii_code
        initial_value *= 17
        initial_value %= 256
...
with open("./data.in") as input_data:
    sequence = input_data.read().strip()
